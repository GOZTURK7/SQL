...
CREATE TABLE calisanlar 
    ...
    select * from calisanlar,markalar;
...
/*  ORNEK2: marka_id’si 101’den büyük olan marka çalişanlarinin isim, maaş ve 
  ...
  -----------------------------------------------------------------------------*/
-- in yerine = (eşittir) olmaz çünkü eşittir bir adet veri için kullanilabilir.
select isim, maas, sehir from calisanlar
where isyeri in (select marka_isim from markalar where marka_id > 101);  -- aslinda burasi isyeri = lcw or isyeri = adidas mantiği ama bu şekilde yazamiyoruz o yüzde in kullandik)
...
==============================================================================*/   
 ...
 (select Sum(maas) from calisanlar where markalar.marka_isim=calisanlar.isyeri) as toplam_maas from markalar; -- as yazmasak da olur.
-- markalar. ve calisanlar. : ifadelerini ayni isimler olsaydi kullaniriz burda ornekledik
-- as toplam_maas : parantes icine isim vermek icin
...
/* -----------------------------------------------------------------------------
 ...
 select marka_isim, calisan_sayisi, 
 (select round(avg(maas)) from calisanlar where markalar.marka_isim = calisanlar.isyeri) ortalama_maas 
 from markalar;
...
(select min(maas) from calisanlar where marka_isim=isyeri) min_maas from markalar;
