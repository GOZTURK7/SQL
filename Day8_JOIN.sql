/*============================== JOIN İSLEMLERİ ===============================
    ...
    2) LEFT JOIN:  Ilk tabloda (Sol) olan sonuclari gosterir s(A)
    3) RIGHT JOIN: sağdaki tabloda olan sonuclari gosterir s(B)
    s(A) + s(B) -s(A kesişim B) = left union right demek aslnda (union kesişimleri atiyor)
	FULL JOIN=(left join + union all + right join) Tablodaki tum sonuclari gosterir
==============================================================================*/   
...
/*=============================== INNER JOIN  ==================================
    ...
    ilk yazlan tablo sol joinden sonra yazilan tablo sağ'daki tablo oluyor.
    
    ON şart olarak kullanyoruz where gibi
...
on sir.sirket_id = sip.siparis_id; 
...
------------------------------------------------------------------------------*/  
    ...
    SELECT sir.sirket_isim, sip.siparis_id, sip.siparis_tarihi, sip.sirket_id
    FROM siparisler sip  -- tablo önceliği farketmez
    INNER JOIN sirketler sir 
    on sip.sirket_id = sip.sirket_id; -- where yerine on yazilacak
...
/*=============================== LEFT JOIN  ==================================
    ...
    LEFT JOIN, 1. tablodan (sol tablo)(from'dan sonra yazilan) SELECT ile ifade edilen sutunlara ait tum
    satirlari getirir. 
...
------------------------------------------------------------------------------*/
 ...
 -- bağlama işi joinlerde on keyword'ü ile yapilir.
