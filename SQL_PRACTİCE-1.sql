...
UPDATE PEOPLE
SET name= COALESCE (name, 'daha sonra eklenecek'); -- tek tek name'lere bakacak benim koyduğum para metreyi 
...
-- Veli Han'ın maaşını en yüksek maaştan yüzde 20 daha artırin
update worker
...
update worker 
set worker_salary=(select max(worker_salary))*(8/10) + worker_salary 
where worker_name="veli han";


-- Ali Can'ın maaşını en düşük maaşin % 30'u oraninda düşürun
update worker
set worker_salary = (select min(worker_salary) from (select * from worker) as W)*0.7
...
update worker
set worker_salary = (select avg(worker_salary) from (select * from worker) as A)
...
select turno, min(sayfasayisi) min_sayfa from kitap
group by turno
order by turno;
...
select (select kitap_turu from islemler where islemler.turno=kitap.turno) kitap_turu, count(idno) kitap_sayisi from kitap 
group by turno;
